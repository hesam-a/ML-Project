...
nn_small_tal = tf.keras.models.Sequential([
    layers.Dense(500, activation="relu", kernel_regularizer=l2(0.0001),input_shape =(399,1567)),#input_shape = metlin_X.shape[1:],kernel_regularizer=l2(0.0001)),#kernel_initializer="he_normal"
...
#small_tal_X, small_tal_tst_X = standardize(small_train_set[:,:-1],small_test_set[:,:-1])
#small_tal_X = standardize(small_train_set[:,:-1])
#small_tal_Y = small_train_set[:,-1]
#small_tal_tst_Y = small_test_set[:,-1]
...
#    b = np.reshape(small_tal_tst_Y,(134,1))
#    slope_tcn, intcept_tcn, rval_tcn, pval_tcn, stderr_tcn= stats.mstats.linregress(a,b)
#    r2_val_file = open("r2_hilic_vals.txt",'a')
